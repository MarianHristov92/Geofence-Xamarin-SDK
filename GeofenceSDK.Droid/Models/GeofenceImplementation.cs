...
using GeofenceSDK.Droid.Services;
...
namespace GeofenceSDK.Droid.Models
{
    ...
    /// </summary>
    /// 
    public class GeofenceImplementation : Java.Lang.Object, GeofenceSDK.Common.IGeofence, IResultCallback, Android.Gms.Tasks.IOnCompleteListener, GoogleApiClient.IConnectionCallbacks, GoogleApiClient.IOnConnectionFailedListener
    {
            ...
            IsLocationEnabled((bool locationIsEnabled) => {
                if (locationIsEnabled)
                {
                    ...
                    if (IsMonitoring)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}", GeofenceCrossService.Id, "Monitoring was restored"));
                    }
                }
                else
                {
                    string message = string.Format("{0} - {1}", GeofenceCrossService.Id, "You need to enabled Location Services");
                    System.Diagnostics.Debug.WriteLine(message);
                    GeofenceCrossService.GeofenceListener.OnError(message);
                }
            });
        }
        ...
        public void IsLocationEnabled(Action<bool> returnAction)
        {
            lock (Lock)
            {
                ...
                switch (GeofenceCrossService.GeofencePriority)
                {
                ...
                mLocationRequest.SetInterval(GeofenceCrossService.LocationUpdatesInterval);
                mLocationRequest.SetFastestInterval(GeofenceCrossService.FastestLocationUpdatesInterval);
        ...
        public void AddGeofences()
        {
            try
            {
                ...
                foreach (GeofenceCircularRegion region in regionsClone)
                {
                    ...
                    if (transitionTypes != 0)
                    {
                        ...
                        .Build());
                        GeofenceCrossService.GeofenceListener.OnMonitoringStarted(region.Id);
                    }
                }
            ...
            {
                string message = string.Format("{0} - Error: {1}", GeofenceCrossService.Id, ex1.ToString());
                System.Diagnostics.Debug.WriteLine(message);
                GeofenceCrossService.GeofenceListener.OnError(message);
            }
            catch (System.Exception ex2)
            {
                string message = string.Format("{0} - Error: {1}", GeofenceCrossService.Id, ex2.ToString());
                System.Diagnostics.Debug.WriteLine(message);
                GeofenceCrossService.GeofenceListener.OnError(message);
            }
        }
        ...
        internal void OnMonitoringRemoval()
        {
            if (mRegions.Count == 0)
            {
                ...
                GeofenceCrossService.GeofenceListener.OnMonitoringStopped();
        ...
        private void RemoveGeofences(IList<string> regionIdentifiers)
        {
            foreach (string identifier in regionIdentifiers)
            {
                ...
                GeofenceCrossService.GeofenceListener.OnMonitoringStopped(identifier);
            }
        ...
        private void InitializeGoogleAPI()
        {
            ...
            if (queryResult == ConnectionResult.Success)
            {
                if (mGoogleApiClient == null)
                {
                    ...
                    string message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Google Play services is available.");
                    System.Diagnostics.Debug.WriteLine(message);
                }
            ...
            else
            {
                ...
                string message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Google Play services is unavailable.");
                System.Diagnostics.Debug.WriteLine(message);
                GeofenceCrossService.GeofenceListener.OnError(message);
            }
        }
        ...
        public void OnConnectionFailed(Android.Gms.Common.ConnectionResult result)
        {
            ...
            string message = string.Format("{0} - {1} {2}", GeofenceCrossService.Id, "Connection to Google Play services failed with error code ", errorCode);
            System.Diagnostics.Debug.WriteLine(message);
            GeofenceCrossService.GeofenceListener.OnError(message);
        ...
        internal void SetLastKnownLocation(Android.Locations.Location location)
        {
            if (location != null)
            {
                ...
                GeofenceCrossService.GeofenceListener.OnLocationChanged(eventLoc);
            }
        ...
        public void OnResult(Java.Lang.Object result)
        {
            ...
            switch (res.Status.StatusCode)
            {
                ...
                case Android.Gms.Location.GeofenceStatusCodes.Success:
                    if (CurrentRequestType == RequestType.Add)
                    {
                        message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Successfully added Geofence.");
                    ...
                    {
                        message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Geofence Update Received");
                    }

                    break;
                case Android.Gms.Location.GeofenceStatusCodes.Error:
                    message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Error adding Geofence.");
                    break;
                case Android.Gms.Location.GeofenceStatusCodes.GeofenceTooManyGeofences:
                    message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Too many geofences.");
                    break;
                case Android.Gms.Location.GeofenceStatusCodes.GeofenceTooManyPendingIntents:
                    message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Too many pending intents.");
                    break;
                case Android.Gms.Location.GeofenceStatusCodes.GeofenceNotAvailable:
                    message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Geofence not available.");
                    break;
            }
            ...
            if (statusCode != Android.Gms.Location.GeofenceStatusCodes.Success && statusCode != Android.Gms.Location.GeofenceStatusCodes.SuccessCache && IsMonitoring)
            {
                ...
                ((GeofenceImplementation)GeofenceCrossService.Current).LocationHasError = true;
        ...
        {
            string message = string.Format("{0} - {1} {2}", GeofenceCrossService.Id, "Connection to Google Play services suspended with error code ", cause);
            System.Diagnostics.Debug.WriteLine(message);
            GeofenceCrossService.GeofenceListener.OnError(message);
        }
        ...
        internal void StartLocationUpdates()
        {
            ...
            mLocationRequest.SetInterval(GeofenceCrossService.LocationUpdatesInterval == 0 ? 30000 : GeofenceCrossService.LocationUpdatesInterval);
            mLocationRequest.SetFastestInterval(GeofenceCrossService.FastestLocationUpdatesInterval == 0 ? 5000 : GeofenceCrossService.FastestLocationUpdatesInterval);
            ...
            System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}: {2}", GeofenceCrossService.Id, "Priority set to", priorityType));
            ...
            if (GeofenceCrossService.SmallestDisplacement > 0)
            {
                mLocationRequest.SetSmallestDisplacement(GeofenceCrossService.SmallestDisplacement);
                System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}: {2} meters", GeofenceCrossService.Id, "Location smallest displacement set to", GeofenceCrossService.SmallestDisplacement));
            }
            ...
            catch (System.Exception e)
            {
                ...
                GeofenceCrossService.GeofenceListener.OnError(e.Message);
            }
        }
