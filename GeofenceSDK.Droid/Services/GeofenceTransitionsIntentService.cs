...
namespace GeofenceSDK.Droid.Services
{
    ...
    public class GeofenceTransitionsIntentService : IntentService
    {
        ...
        protected override void OnHandleIntent(Intent intent)
        {
            try
            {
                lock (IntentLock)
                {
                    ...
                    if (geofencingEvent.HasError)
                    {
                        ...
                        string message = string.Format("{0} - {1}", GeofenceCrossService.Id, errorMessage);
                    ...
                    if (triggeringGeofences != null)
                    {
                        foreach (Android.Gms.Location.IGeofence geofence in triggeringGeofences)
                        {
                            ...
                            {
                                ((GeofenceImplementation)GeofenceCrossService.Current).CurrentRequestType = GeofenceImplementation.RequestType.Update;
                                if (!GeofenceCrossService.Current.GeofenceResults.ContainsKey(geofence.RequestId))
                                {
                                    ((GeofenceImplementation)GeofenceCrossService.Current).AddGeofenceResult(geofence.RequestId);
                                }
                                var result = GeofenceCrossService.Current.GeofenceResults[geofence.RequestId];
                                ...
                                switch (geofenceTransition)
                                {
                                    ...
                                    case Android.Gms.Location.Geofence.GeofenceTransitionDwell:
                                        ...
                                        break;
                                    default:
                                        string message = string.Format("{0} - {1}", GeofenceCrossService.Id, "Invalid transition type");
                                        System.Diagnostics.Debug.WriteLine(message);
                                        gTransition = GeofenceTransition.Unknown;
                                        break;
                                }
                                System.Diagnostics.Debug.WriteLine(string.Format("{0} - Transition: {1}", GeofenceCrossService.Id, gTransition));
                                if (result.Transition != gTransition)
                                {
                                    result.Transition = gTransition;
                                    geofenceTransitions.Add(new GeofenceResult(result));
                                    if (GeofenceCrossService.Current.Regions.ContainsKey(result.RegionId))
                                    {
                                        var region = GeofenceCrossService.Current.Regions[result.RegionId];
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        foreach (var result in geofenceTransitions)
                        {
                            ...
                            GeofenceCrossService.GeofenceListener.OnRegionStateChanged(result);
            ...
            {
                System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}", GeofenceCrossService.Id, ex.ToString()));
            }
        ...
        public void CreateNotification(string title, string message)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (GeofenceCrossService.IconResource == 0)
                    {
                        GeofenceCrossService.IconResource = context.ApplicationInfo.Icon;
                    }
                    else
                    {
                        string name = context.Resources.GetResourceName(GeofenceCrossService.IconResource);
                ...
                builder = new NotificationCompat.Builder(context)
                        ...
                        .SetSmallIcon(GeofenceCrossService.IconResource) // This is the icon to display
                        .SetContentText(message); // the message to display.

                // Set the color if we have one
                if (GeofenceCrossService.Color != 0)
                    builder.SetColor(GeofenceCrossService.Color);
            ...
            {
                System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}", GeofenceCrossService.Id, ex1.ToString()));
            }
